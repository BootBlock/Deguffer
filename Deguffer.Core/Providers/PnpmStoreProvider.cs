...
public sealed class PnpmStoreProvider : CleanupProviderBase
{
    ...
    public override ProviderDescription Description { get; } = new()
    {
        ...
        Recommendation = "Yes. Deguffer runs pnpm's own store prune, which removes only the "
            + "packages no project on the machine still links, so nothing an installed project "
            + "depends on is taken.",
    };
