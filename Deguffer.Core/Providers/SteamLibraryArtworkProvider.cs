...
/// games on the machine it was measured on, with files dating back years: nothing Steam does evicts
/// it.
///
/// <para><b>Tier 1, on an observation rather than a claim.</b> Folders for three games were moved
/// out of a real client's cache with Steam closed, leaving Steam's own index,
...
///
/// <para><b>Its own row rather than a part of <see cref="SteamCacheProvider"/>, for the one thing
/// that makes it more than a cache.</b> Replacing these files by hand is a long-standing way of
/// changing a game's library artwork. Steam's supported route writes to <c>userdata</c>, which is
/// never reached here, but a hand-edited file in this folder cannot be told from a downloaded one.
/// So each game is an item that can be kept, and the row says so rather than the web cache's.</para>
///
...
public sealed class SteamLibraryArtworkProvider : CleanupProviderBase
{
    ...
    public override ProviderDescription Description { get; } = new()
    {
        ...
        Purpose = "Steam downloads the capsule, banner and logo it shows for every game in your "
            + "library, including games you do not have installed, and keeps them per game. It never "
            + "removes any of them.",
        Recommendation = "Safe to clear: Steam downloads a game's artwork again when it next shows "
            + "the game. Deguffer removes the folders for single games inside Steam's artwork folder "
            + "and nothing else in Steam's folder.",
    };
    ...
    public override IReadOnlyList<ToolRoot> ToolRoots => _toolRoots ??=
        _discovery.Install.Root is { } install
            ? [new ToolRoot(
                ...
                name => SteamAppId.IsAppId(name) || FlatArtworkAppId(name) is not null)]
            : [];
