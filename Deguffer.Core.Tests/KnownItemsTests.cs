...
public sealed class KnownItemsTests : IDisposable
{
    ...
    /// refuses. No provider targets them, and the Storage page names a store only to say it is left
    /// alone, so this is the only place the app says what a forty-gigabyte file Explore will not remove
    /// actually is — and it has to say it about an archive on a data disk as much as about one in the
    /// folder Outlook suggested.
    /// </summary>
    [Theory]
