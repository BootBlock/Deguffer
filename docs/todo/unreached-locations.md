...
> shader caches and Steam shader pre-cache, §6's crash dumps, servicing logs and completed upgrade
> scaffolding, §7's per-volume recycle bins, §8's Unreal Engine derived-data caches, §10's release of
> OneDrive's local copies and §12's Squirrel staging and superseded builds have shipped; everything
> else is unstarted. **Open questions 1, 2 and 3 are answered** — see the foot of this document.
