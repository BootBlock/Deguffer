...
- **Hover text says what one of Windows' own parts is, and why the size it is drawn at is normal.**
  ...
  safe to close, and it never suggests closing anything. It is a tooltip over the picture rather than
  a line under it: several sentences of wrapped text under the picture reflow the page on every shape
  the pointer crosses, which moves the shape out from under the pointer.
...
| A process at a higher integrity level than Deguffer's own, or whose level will not be read | "The thread of a process can post messages only to message queues of threads in processes of lesser or equal integrity level" ([PostMessage](https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-postmessagew)), and Microsoft publishes no list of what passes the filter, only that write-type messages do not ([UIPI](https://learn.microsoft.com/en-us/previous-versions/dotnet/articles/bb625963(v=msdn.10))). The current `PostMessage` page says a blocked post sets the last error to 5, while the archived design note says a blocked call "return[s] success but silently drop[s] the window message". Two Microsoft sources disagree about what the post reports, so Deguffer decides before posting and never by what the post reports. Deguffer therefore reads the level from the token's mandatory label ([TOKEN_MANDATORY_LABEL](https://learn.microsoft.com/en-us/windows/win32/api/winnt/ns-winnt-token_mandatory_label)) and compares its value against Deguffer's own, never by equality and never by named band. Windows states that the levels are an ordered scale of numbers, where "a lower value indicates a lower integrity level", and gives a UIAccess program the medium level "plus an increment of 0x10" ([Windows Integrity Mechanism Design](https://learn.microsoft.com/en-us/previous-versions/dotnet/articles/bb625963(v=msdn.10))). That increment exists to rank such a program above plain medium, so a comparison that grouped both into one band would call them equal and post to a window the filter blocks, while a comparison by value ranks every level between two named ones correctly. The named values are the well-known RIDs ([Well-known SIDs](https://learn.microsoft.com/en-us/windows/win32/secauthz/well-known-sids)). |
